...
var (
	...
	clusterTypes = ordererTypes{"etcdraft"}
)
...
func Start(cmd string, conf *localconfig.TopLevel) {
	...
	tlsCallback := func(bundle *channelconfig.Bundle) {
		...
		if grpcServer.MutualTLSRequired() {
			...
			updateTrustedRoots(grpcServer, caSupport, bundle, clusterDialer, clusterConfig.SecOpts.ServerRootCAs)
		}
	}
	manager := initializeMultichannelRegistrar(clusterDialer, serverConfig, grpcServer, conf, signer, tlsCallback)
...
func isClusterType(genesisBlock *cb.Block) bool {
	...
	return clusterTypes.exists(ordConf.ConsensusType())
}
...
func initializeMultichannelRegistrar(clusterDialer *cluster.PredicateDialer, srvConf comm.ServerConfig,
	srv *comm.GRPCServer,
	...
	if isClusterType(genesisBlock) {
		raftConsenter := newEtcdRaftConsenter(clusterDialer, conf, srvConf, srv, registrar)
...
func newEtcdRaftConsenter(clusterDialer *cluster.PredicateDialer, conf *localconfig.TopLevel,
	srvConf comm.ServerConfig, srv *comm.GRPCServer, r *multichannel.Registrar) *etcdraft.Consenter {
...
func createComm(clusterDialer *cluster.PredicateDialer,
	conf *localconfig.TopLevel,
	c *etcdraft.Consenter) *cluster.Comm {
	comm := &cluster.Comm{
		Logger:       flogging.MustGetLogger("orderer/common/cluster"),
...
func updateTrustedRoots(srv *comm.GRPCServer, rootCASupport *comm.CASupport,
	cm channelconfig.Resources, clusterDialer *cluster.PredicateDialer, localClusterRootCAs [][]byte) {
	rootCASupport.Lock()
