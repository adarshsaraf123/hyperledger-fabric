...
func (bh *Handler) ProcessMessage(msg *cb.Envelope, addr string) (resp *ab.BroadcastResponse) {
	...
	chdr, isConfig, channelSupport, err := bh.SupportRegistrar.BroadcastChannelSupport(msg)
	if chdr != nil {
	...
	if !isConfig {
		...
		configSeq, err := channelSupport.ProcessNormalMsg(msg)
		if err != nil {
		...
		if err = channelSupport.WaitReady(); err != nil {
			logger.Warningf("[channel: %s] Rejecting broadcast of message from %s with SERVICE_UNAVAILABLE: rejected by Consenter: %s", chdr.ChannelId, addr, err)
			return &ab.BroadcastResponse{Status: cb.Status_SERVICE_UNAVAILABLE, Info: err.Error()}
		}

		err = channelSupport.Order(msg, configSeq)
		if err != nil {
		...
		config, configSeq, err := channelSupport.ProcessConfigUpdateMsg(msg)
		if err != nil {
		...
		err = channelSupport.Configure(config, configSeq)
		if err != nil {
