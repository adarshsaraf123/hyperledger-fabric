...
loop:
	...
	second_loop:
		...
		// validate the specified members for FSMConfig
		if ord.EtcdRaft.FSMConfig.ElectionTick <= ord.EtcdRaft.FSMConfig.HeartbeatTick {
			logger.Panicf("election tick must be greater than heartbeat tick")
		}

		for _, c := range ord.EtcdRaft.GetConsenters() {
			// TODO: should we also panic if the host ip/port is not specified?
